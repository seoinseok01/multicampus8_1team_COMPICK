...
public class RecoveryController {
    ...
    public String sendFindId(@RequestParam String name, @RequestParam String email,
        ...
        try {
            ...
            session.removeAttribute("findIdName"); session.removeAttribute("findIdEmail");
        }
    ...
    public String confirmFindId(@RequestParam String code, HttpSession session, Model model) {
        ...
        if (name == null || email == null) { model.addAttribute("error", "먼저 인증번호를 요청해 주세요."); return "member/find-id"; }
        try {
    ...
    public String sendPasswordReset(@RequestParam String loginId, @RequestParam String email,
        ...
        try {
            ...
            session.removeAttribute("resetLoginId"); session.removeAttribute("resetEmail");
        }
        redirect.addFlashAttribute("message", "입력한 정보와 일치하는 계정이 있다면 인증 메일을 전송했습니다.");
    ...
    public String confirmPasswordReset(@RequestParam String code, @RequestParam String newPassword,
        ...
        if (loginId == null || email == null) { model.addAttribute("error", "먼저 인증번호를 요청해 주세요."); return "member/password-reset"; }
        try {
            if (!newPassword.equals(newPasswordConfirm)) throw new IllegalArgumentException("새 비밀번호가 일치하지 않습니다.");
        ...
        } catch (IllegalArgumentException e) { model.addAttribute("error", e.getMessage()); return "member/password-reset"; }
    }
}
