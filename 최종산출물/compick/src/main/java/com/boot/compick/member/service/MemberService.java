...
import com.boot.compick.member.dto.JoinForm;
import com.boot.compick.member.dto.PasswordForm;
...
public class MemberService {
    ...
    public String findLoginId(String phone, String rawEmail) {
        ...
        Member member = memberRepository.findByEmailAndStatus(email, MemberStatus.ACTIVE)
                .filter(value -> value.getPhone().replaceAll("\\D", "").equals(normalizedPhone))
    ...
    public void validatePasswordResetMember(String loginId, String rawEmail) {
        memberRepository.findByLoginIdAndEmailAndStatus(loginId, rawEmail.trim().toLowerCase(Locale.ROOT), MemberStatus.ACTIVE)
                .orElseThrow(() -> new IllegalArgumentException("일치하는 회원정보가 없습니다."));
    }
    ...
    public void resetPassword(String loginId, String rawEmail, String newPassword, String confirm) {
        ...
        if (!newPassword.matches("^(?=.*[A-Za-z])(?=.*\\d)(?=.*[^A-Za-z0-9]).{8,}$")) throw new IllegalArgumentException("비밀번호는 영문, 숫자, 특수문자를 포함한 8자 이상이어야 합니다.");
        Member member = memberRepository.findByLoginIdAndEmailAndStatus(loginId, rawEmail.trim().toLowerCase(Locale.ROOT), MemberStatus.ACTIVE)
                .orElseThrow(() -> new IllegalArgumentException("일치하는 회원정보가 없습니다."));
    ...
    public void updateProfile(String loginId, ProfileForm form) {
        findActiveByLoginId(loginId).updateProfile(form.getName(), form.getPhone());
    }

    @Transactional
    public void changePassword(String loginId, PasswordForm form) {
        Member member = findActiveByLoginId(loginId);
        if (!passwordEncoder.matches(form.getCurrentPassword(), member.getPasswordHash())) {
